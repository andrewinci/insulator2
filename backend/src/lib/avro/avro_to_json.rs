...
impl<S: SchemaProvider> AvroParser<S> {
    pub async fn avro_to_json(&self, raw: &[u8]) -> AvroResult<String> {
        ...
        let record = from_avro_datum(&schema.schema, &mut data, None).map_err(|err| AvroError::ParseAvroValue(err))?;
        let json = map(&record, &schema.schema, &None, &schema.resolved_schemas)?;
        let res = serde_json::to_string(&json).map_err(|err| AvroError::ParseJsonValue(err))?;
        Ok(res)
    }
}

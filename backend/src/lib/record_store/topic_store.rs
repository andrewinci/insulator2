...
impl<S: RecordStore, P: KafkaRecordParser> TopicStore<S, P> {
    ...
    pub fn setup(&self, compactify: bool) -> LibResult<()> {
    ...
    ) -> LibResult<Vec<ParsedKafkaRecord>> {
    ...
    pub async fn insert_record(&self, record: &RawKafkaRecord) -> LibResult<()> {
    ...
    pub fn get_records_count(&self) -> LibResult<usize> {
        Ok(*self.records_counter.read().unwrap())
    }

    pub fn export_records(&self, options: &ExportOptions) -> LibResult<()> {
        let ExportOptions {
            limit,
            query,
            output_path,

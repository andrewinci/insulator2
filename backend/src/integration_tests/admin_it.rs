...
mod integration_tests {
    ...
    async fn produce_and_consume_messages() {
        ...
        for (i, message) in expected.iter().enumerate() {
            producer
                .send(
                    FutureRecord::to(topic).payload(message).key(&format!("Key {}", i)),
        ...
        consumer.subscribe(&[topic]).expect("Failed to subscribe to a topic");
        ...
        for produced in expected {
            ...
            assert_eq!(
                produced,
                borrowed_message.unwrap().payload_view::<str>().unwrap().unwrap()
            );
        }
    }
}
